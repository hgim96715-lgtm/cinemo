...
TMDB의 `release_date`는 후보 날짜이고, 오늘 개봉 확정 여부는 다음 순서로 판단함.

```txt
KOBIS 개봉일 매칭 성공
  → 오늘 개봉

원개봉일과 현재 개봉일이 1년 이상 차이
  → 재개봉으로 간주
  → 오늘 개봉 (재개봉)

TMDB에 오늘 날짜만 있고 위 조건이 없음
  → 개봉일 확인 중
...
KOBIS `searchMovieList`의 `openStartDt`·`openEndDt`는 `YYYY-MM-DD`가 아니라 연도 4자리만 받음. 연도 범위로 조회한 뒤 응답의 `openDt`를 실제 날짜 범위와 비교해 필터링함.

합본·재상영 영화는 TMDB에서 작품별 카드로 남길 수 있음. 이 경우 원개봉일이 확인되면 각 카드에 같은 재개봉 상태를 표시하고, 국내 개봉 근거가 없는 영화는 `개봉일 확인 중`으로 남김.

`개봉일 확인 중` 항목은 확정된 개봉작과 섞이지 않도록 목록 마지막으로 정렬함. `더 보기`로 추가된 항목까지 기존 목록과 합쳐 다시 정렬함.
