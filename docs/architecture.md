...
Web에서 검증이 통과해도 브라우저 요청은 신뢰하지 않음. API DTO와 Service에서 입력·권한·도메인 규칙을 다시 확인한 뒤 Prisma에 전달함. 자세한 개념은 [폼 검증 문서](./concepts/form-validation.md) 참고.
...
| 기능 모듈 | `UserMovieModule` | 개인 영화 활동과 개봉일 알림·Cron 담당 |
...
`MOVIE CHART`처럼 HTTP API의 요청·응답 DTO는 `shared`에 다시 정의하지 않고 `packages/api-contract`의 OpenAPI 생성 타입을 사용함. 상세 기준은 [API 계약과 OpenAPI codegen](./concepts/api-contract.md) 참고.
...
flowchart LR
  ...
  Notify --> Cron[NestJS Cron]
  Cron --> TMDB[TMDB release date 확인]
  Cron --> Email[React Email + Resend]
...
Daily Excel → artifact 보관 때문에 GitHub Actions 유지
```
