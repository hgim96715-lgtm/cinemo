...
소셜 계정       SocialAccount       provider + providerAccountId · User 연결
OAuth 교환      OAuthLoginCode      codeHash · expiresAt · consumedAt · 일회용
비밀번호 재설정  PasswordResetToken  tokenHash · expiresAt · usedAt · 일회용 재설정 토큰
...
| **SocialAccount**    | Google·Naver·Kakao·Apple provider 계정 연결 · `(provider, providerAccountId)` unique |
| **OAuthLoginCode**   | OAuth callback과 Web 사이의 일회용 교환 code · User FK · 만료·소비 상태 |
| **PasswordResetToken** | 이메일 비밀번호 재설정 토큰 · `tokenHash`만 저장 · 만료·사용 상태 |
| **Ticket**           | 오늘 1회권 · `machine_id`/`tmdb_id`로 뽑기 결과도 보관             |
| **Draw**             | 뽑기 결과(영화+쪽지). Ticket 1:1 · **추후 분리**                   |
| **UserMovie**        | ♥ 찜 · 봤어요 · 관람일 · 가챠 watched 제외 · 선반 목록 · MY CINEMA 포스터 전시 |
...
## UserMovie — 찜·봤어요·관람 기록

테이블 `user_movies`. Web 상세 → [web/my-cinema.md](../web/my-cinema.md)

```txt
id · userId · tmdbId
kind: wish | watched
watchedAt: DateTime?              // watched 전용 관람일, Timestamptz
viewingType: theater | home | other
viewingTypeCustom: String?        // viewingType=other일 때만 사용
viewingPlatform: String?
viewingLocation: String?
review: Text?
rating: SmallInt?                 // 1~10
isDisplayed · displayOrder · wallSlot
createdAt · updatedAt
```

- `@@unique([userId, tmdbId, kind])`로 사용자·영화·종류별 한 행만 유지함
- wish와 watched는 별도 행으로 동시에 존재할 수 있음
- 현재 정책은 영화 1편당 관람일 1개이며, 반복 관람 이력은 별도 모델 범위임
- `watchedAt` 입력·월 범위·통계 집계는 KST 기준으로 처리함
- `viewingType`이 `other`가 아니면 `viewingTypeCustom`을 비움
- `tmdbId`로 `MoviePool`과 논리 연결하며 포스터 이미지 자체는 저장하지 않음
- `wallSlot`은 MY CINEMA 포스터 전시 위치 1~3을 의미함

주요 API:

```txt
GET    /v1/user-movies?kind=watched&search=&year=&month=
POST   /v1/user-movies/watched-at
PATCH  /v1/user-movies/watched-at
DELETE /v1/user-movies/watched-at/:tmdbId
PATCH  /v1/user-movies/viewing-details
GET    /v1/user-movies/calendar?year=&month=
GET    /v1/user-movies/stats?year=
```
...
Web        /quote · /my-cinema/quotes · quote-api.ts · components/quote/
```
...
- `User.lastLoginProvider` — 최근 로그인 방식 `email | google | naver | kakao | apple`
- `SocialAccount` — provider와 provider 고유 계정 ID 조합 unique · 이메일만으로 계정을 자동 병합하지 않음
- `OAuthLoginCode` — 원문 대신 `codeHash` 저장 · 짧은 만료 시간 · `consumedAt`으로 일회용 보장
...
- `UserMovie.isDisplayed` · `displayOrder` · `wallSlot` — 봤어요 영화의 MY CINEMA 포스터 전시 상태·순서·위치(1~3)
...
- UserMovie API → [web/my-cinema.md](../web/my-cinema.md)
