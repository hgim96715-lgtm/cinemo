...
브라우저에서 직접 `.ics`를 생성하는 [apps/web/lib/movie-calendar.ts](../../apps/web/lib/movie-calendar.ts)는 향후 다운로드 방식 전환 시 재사용 대상. 현재 상세 모달은 서버의 `/api/calendar/movie` route 사용.
...
접근성 속성의 상세 규칙은 [concepts/aria.md](../concepts/aria.md)에 기록.
...
```mermaid
flowchart LR
  Cron[매일 09:00 Asia/Seoul NestJS Cron]
  Cron --> Query[enabled=true · sentAt=null · releaseDate<=오늘]
  Query --> TMDB[TMDB 영화 제목 조회]
  TMDB --> Email[React Email + Resend 발송]
  Email --> SentAt[sentAt 기록]
```

- 실행 주체: GitHub Actions가 아닌 NestJS `@Cron()`
...
`POST /v1/release-notifications/cron`은 수동 테스트용 endpoint.

- `x-cron-secret` 헤더 필요
- 값은 `apps/api/.env`의 `NEST_CRON_SECRET`과 일치
