...
flowchart LR
  ...
  Actions[GitHub Actions] --> ReleaseAPI[개봉일 알림 API]
  ReleaseAPI --> Mail[MailService]
...
sequenceDiagram
  ...
  participant API as ReleaseNotificationController
  participant Service as ReleaseNotificationService
  participant DB as PostgreSQL
  participant TMDB as TmdbService
  participant Mail as MailService
  participant Resend as Resend

  Actions->>API: POST /v1/release-notifications/cron + x-cron-secret
  API->>Service: sendDueReleaseNotifications()
  Service->>DB: enabled=true · sentAt=null · releaseDate<=오늘
  Service->>TMDB: 영화 제목 조회
  Service->>Mail: 제목·닉네임·개봉일 전달
  Mail->>Resend: HTML 이메일 발송
  Resend-->>Mail: 발송 결과
  Mail-->>Service: 성공 반환
  Service->>DB: sentAt 기록
```

- 실행 주체: GitHub Actions `.github/workflows/release-notification.yml`
- 실행 시각: 매일 KST 오전 9시, workflow 기준 UTC 00:00
- 호출 endpoint: `POST /v1/release-notifications/cron`
- 인증 방식: GitHub Secret `CINEMO_CRON_SECRET`을 `x-cron-secret` 헤더로 전달. 값은 Railway의 `CRON_SECRET`과 일치해야 함
...
개봉일 알림의 저장·상태 API·Actions 및 Swagger 수동 실행은 [../lobby/upcoming.md](../lobby/upcoming.md#개봉일-알림) 참고.
...
1. 영화 상세에서 `보고 싶어요` 저장
2. 개봉일 알림 활성화
3. Swagger에서 `POST /v1/release-notifications/cron`을 실행하거나 GitHub Actions의 `workflow_dispatch`로 실행
4. 알림 대상 사용자의 이메일 수신함 확인

수동 실행에는 `x-cron-secret` 헤더와 API 환경변수 `CRON_SECRET` 값이 필요하다. 테스트 조건과 endpoint는 [lobby/upcoming.md](../lobby/upcoming.md#개봉일-알림) 참고.
...
5. 개봉일 알림은 알림 설정 후 Swagger endpoint, Actions 수동 실행 또는 다음 정기 실행으로 확인
...
| 개봉일 알림 실행 | NestJS `Logger` 기록 후 다음 알림 계속 처리 |
